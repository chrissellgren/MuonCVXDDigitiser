...
{ 
    ...
    streamlog_out(DEBUG0) << "  Parameters printed  " << std::endl ;
...
{
    streamlog_out(DEBUG0) << "Loading geometry..." << std::endl;
    Detector& theDetector = Detector::getInstance();
    streamlog_out(DEBUG0) << "Got detector instance." << std::endl;
    ...
    streamlog_out(DEBUG0) << "Got subdetector." << std::endl; 
    ...
    streamlog_out(DEBUG0) << "Got zPlanarData." << std::endl;        
    ...
    streamlog_out(DEBUG0) << "Got detector layers." << std::endl;
    ...
    for(ZPlanarData::LayerLayout z_layout : detector_layers)
    {
        ...
        streamlog_out(DEBUG5) << "ZSegmented is true" << std::endl;
        ...
        streamlog_out(DEBUG0) << "ZSegmented is false " << std::endl;
#endif
    ...
    if (_layerLadderLength[0] == 0 && (_subDetName == "OuterTrackerBarrel")){
    ...
    // shift digitized bins for inner tracker
    if (_subDetName == "InnerTrackerBarrel" || _subDetName == "OuterTrackerBarrel") {
...
{ 
    ...
    streamlog_out(DEBUG0) << "Getting input collection..." << std::endl;
    ...
    if( STHcol != nullptr )
    {
        streamlog_out(DEBUG0) << "Got input collection." << std::endl;
       ...
       relCol->setFlag( lcFlag.getFlag()  ) ;
        ...
        //**************************************************************************    
         ...
         if (_produceFullPattern != 0) {
            if (_subDetName == "VertexBarrel" || _subDetName == "VertexEndcap") {
                evt->addCollection(STHLocCol, "VXPixels");
            if (_subDetName == "InnerTrackerBarrel" || _subDetName == "InnerTrackerEndcap") {
                evt->addCollection(STHLocCol, "ITPixels");
                }
            if (_subDetName == "OuterTrackerBarrel" || _subDetName == "OuterTrackerEndcap") {
                evt->addCollection(STHLocCol, "OTPixels");
            }
         }
    }
 ...
 * 
 */
// CS: new coord system for endcap?
...
void MuonCVXDDigitiser::ProduceIonisationPoints(SimTrackerHit *hit)
{
    ...
    streamlog_out( DEBUG0 ) << "Current layer is: " << _currentLayer << std::endl;
    ...
    entry[2] = -_layerHalfThickness[_currentLayer]; //CS: z in local coordinates is radial dir?
